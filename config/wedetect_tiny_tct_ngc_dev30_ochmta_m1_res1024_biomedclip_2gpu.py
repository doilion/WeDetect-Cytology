...
# Row1 baseline @ 1024 (2026-06-07). PLAIN model (ConvNeXt-tiny + frozen BiomedCLIP
# text + organ mask) on NATIVE TCT_NGC images at 1024x1024 -- NO method changes
# (no stride-4 / NWD / distillation). This is the FAIR baseline at the new resolution
# that all later small-cell methods compare against. Cells: native median 137px -> 36px
# at 1024 (vs 22px at 640); ~45% still <32px (room for the coarse-to-fine stride-4 head
# later). Launch with --amp (1024 is ~2.6x the 640 memory; head is the hog).

data_root = _resolve_data_root("TCT_NGC_1024_ROOT", "data/TCT_NGC_1024")  # 1024 cache
...
# Fixed seed for a clean single-run A/B (baseline vs our method use the SAME seed,
# so any delta is attributable to the method, not seed noise). No multi-seed (time).
randomness = dict(seed=42, deterministic=False)

work_dir = "./work_dirs/wedetect_tiny_tct_ngc_dev30_ochmta_m1_res1024_biomedclip_2gpu"
