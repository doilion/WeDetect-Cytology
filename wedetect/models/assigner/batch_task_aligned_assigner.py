...
from .nwd_metric import nwd_similarity, NWD_DEFAULT_C
...
class BatchTaskAlignedAssigner(nn.Module):
    ...
    Args:
        ...
        use_ciou (bool): Whether to use ciou while calculating iou.
            Defaults to False.
        use_nwd (bool): Whether to use the Normalized Wasserstein Distance
            (scale-stable Gaussian metric for tiny objects) in place of IoU for
            the alignment metric AND positive selection. Overrides ``use_ciou``
            when True. Defaults to False.
        nwd_C (float): NWD normalization constant (~ mean object pixel size in
            the input coordinate space). Data-driven; defaults to the TCT_NGC
            dev30 @640 value. See ``nwd_metric.NWD_DEFAULT_C``.
    """
    ...
    def get_box_metrics(self, pred_bboxes: Tensor, pred_scores: Tensor,
        ...
        # NWD (scale-stable Gaussian metric) takes priority over IoU/CIoU; it is a
        # drop-in [bs, num_gt, num_priors] replacement that does not collapse on the
        # ~2.7px-on-P3 tiny cells, so it both drives the alignment metric and (via the
        # returned `overlaps`) the positive selection + score normalization downstream.
        if self.use_nwd:
            overlaps = nwd_similarity(gt_bboxes, pred_bboxes, C=self.nwd_C)
        elif self.use_ciou:
