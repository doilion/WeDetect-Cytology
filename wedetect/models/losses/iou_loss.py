...
from ..assigner.nwd_metric import nwd_similarity_aligned, NWD_DEFAULT_C
...
class mmyoloIoULoss(nn.Module):
    ...
    def __init__(self,
        ...
        assert iou_mode in ('ciou', 'siou', 'giou', 'nwd')
        if iou_mode == 'nwd':
            assert bbox_format == 'xyxy', \
                "nwd regression loss requires bbox_format='xyxy'"
        self.nwd_C = nwd_C
    ...
    def forward(
        ...
        if self.iou_mode == 'nwd':
            # Scale-stable Gaussian-Wasserstein regression term for tiny boxes
            # (pairwise, xyxy). Mirrors `1 - iou` so reduction/weighting is unchanged.
            iou = nwd_similarity_aligned(pred, target, C=self.nwd_C, eps=self.eps)
        else:
            iou = bbox_overlaps(
                pred,
                target,
